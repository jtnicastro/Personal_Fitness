...
def index(request):
    ...
    return render(
        request, "nutritionApp/index.html", {"clients_with_totals": clients_with_totals}
    )
...
def nutritionPage(request, client_username):
    ...
    totalP = (
        (total_daily_protein * 4) + (total_daily_fats * 9) + (total_daily_carbs * 4)
    )
    ...
    return render(
        request,
        "nutritionApp/nutritionPage.html",
        {
            "client": client,
...
def addMeal(request, client_username):
    ...
    return render(
        request,
        "nutritionApp/addMeal.html",
        {"client": client, "meal_form": meal_form, "food_formset": food_formset},
    )
